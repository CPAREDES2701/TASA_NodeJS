...
app.post('/api/preciospesca/CargaArchivo', cors(corsOptionsDelegate),function (req, res) {  
    console.log('Node server has been invoked. Now calling Backend service API ...');
    _getAccessToken()
    .then((result) => {
        console.log('Successfully fetched OAuth access token: ' +  result.accessToken.substring(0,16));
        var sUrl = HOST + "/api/preciospesca/CargaArchivo";
        return _doQUERY(sUrl, result.accessToken, req.body, 'POST');
    })
    .then((result) => {
        console.log('Successfully called OData service. Response body: ' + result.responseBody);
...
// get
app.get('/api/General/VerArchivo', cors(corsOptionsDelegate),function (req, res) {  
    console.log('Node server has been invoked. Now calling Backend service API ...');
    _getAccessToken()
    .then((result) => {
        ...
        console.log('Successfully fetched OAuth access token: ' +  result.accessToken.substring(0,16));
        var sUrl = HOST + "/api/General/VerArchivo?nomImagen=" +  nomImagen;//+ req.query.nomImg;
...
app.post('/api/General/ObtenerDatosAD', cors(corsOptionsDelegate),function (req, res) {
    res.setHeader('Content-Type', 'application/json');  
    console.log('Node server has been invoked. Now calling Backend service API ...');
    _getAccessToken()
    .then((result) => {
        console.log('Successfully fetched OAuth access token: ' +  result.accessToken.substring(0,16));
        var sUrl = HOST + "/api/General/ObtenerDatosAD";
        return _doQUERY2(sUrl, result.accessToken, req.body, 'POST',res);
    })
    .then((result) => {
        console.log('Successfully called OData service. Response body: ' + result.responseBody);
